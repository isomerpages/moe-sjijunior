---
title: IT Support
permalink: /for-parents/it-support/
description: ""
variant: markdown
---
**IT Support for Students & Parents**
-------------------------------------

If you require technical assistance during school days or during Home-Based Learning (HBL), please follow the guidelines below.

* * *

### **Support During School Days**

For all IT-related issues—including locked accounts or password reset requests—please reach out to the **Teacher-in-Charge via ClassDojo**.

**Student Learning Space (SLS)**
**MIMS Account**
**Sjij Gmail Accounts**

### **Support During Home-Based Learning (HBL)**

If support is needed specifically on HBL days, please submit your request through the official channels:

*   **Request Method:** 
Please use the **relevant links** provided on our portal to make your request.

[SlS Account Id and Password Request](https://forms.gle/yryBsPWcBrF3NqV66)

[MIMS ID or Password Reset](https://docs.google.com/forms/d/e/1FAIpQLScwfeV7wRUvy_Cpc_3aNjFxdeHaKPwl0WNOQB5FV9COjKuEeg/viewform?usp=publish-editor)

[Google Account/Classkick Id and Password Request  ](https://forms.gle/mW4KW9L23UT5WHKt9)

    
*   **Processing Time:** Please allow up to **3 working days** for our team to process and reply to your request.

### **School Operating Hours**

Our IT support team is available during the following hours:

*   **Mondays – Fridays:** 8:00 am – 4:00 pm
    
*   _Closed on Saturdays, Sundays, and Public Holidays._
    

* * *

### **Proactive Tip: SLS Self-Reset**

To regain access to SLS instantly without waiting for support, we highly recommend that students set up **Security Questions** within their SLS profile.

*   **The Benefit:** Once configured, students can perform a **self-reset** of their password at any time.
    
*   **How to set it up:** Log in to SLS, navigate to **Profile Settings**, and update the security questions section.

https://www.learning.moe.edu.sg/login-troubleshooting/authentication/reset-sls-password-student/


**Non-School Operating Hours (SLS)**

Parents can only contact the SLS Helpdesk outside the school operating hours.

For other matters, students can email the SLS Helpdesk at [helpdesk@sls.ufinity.com](mailto:helpdesk@sls.ufinity.com). When emailing the SLS Helpdesk, they should include their full name, name of school and form class.

SLS Helpdesk: 6702 6513

Mondays - Fridays: 4:00 pm - 9:00 pm

Saturdays: 9:00 am - 9:00 pm

\*Closed on Sundays & Public Holidays